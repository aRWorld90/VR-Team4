...
def build_ssd(img_shape=(300, 300, 3), n_classes=80, load_pretrained=False, freeze_layers_from='base_model'):

    # SSD model is only tested for TF backend
    ...
    # Freeze some layers
    # TODO: base_model_layers (freeze layers from) => (DONE)
    if freeze_layers_from is not None:
        if freeze_layers_from == 'base_model':

            base_model_layers = ['input_1', 'conv1_1', 'conv1_2', 'pool1',
...
def SSD300(input_shape, num_classes=80):
    ...
    img_size = (input_shape[1], input_shape[0]) # Important
